...
class MobileApplicationClient(Client):
    ...
    client code is downloaded from a webapp server and executes within a
    user-agent (e.g. webapp browser) on the device used by the resource
    ...
    interacting with the resource owner's user-agent (typically a webapp
